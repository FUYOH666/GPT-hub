...
3. В **GPTHub v3** опционально **canned short-circuit** для **`greeting_or_tiny`** без картинок: **`GREETING_CANNED_RESPONSE_ENABLED=true`** (по умолчанию **выключено**); в trace — `canned_response: true`, вызова LiteLLM нет. Остальные запросы при **`AUTO_ROUTE_MODEL=true`** маршрутизируются по ролям; короткие реплики — роль **`fast_text_chat`** с цепочкой **`gpt-hub-turbo` → `gpt-hub-fallback`**. System-промпты описывают только **внешнее** поведение (язык, длина, формат), без «never / do not / constraints», чтобы модель не цитировала мета-инструкции. В селекторе при **`single_public`** пользователь видит **`gpt-hub`**; фактический алиас — в trace. Если **`AUTO_ROUTE_MODEL=false`**, запрос с **`gpt-hub`** маппится на **`default_text_model`** оркестратора (по умолчанию **`gpt-hub-turbo`**), т.к. в LiteLLM нет id `gpt-hub`.
