...
- **v3 orchestrator — UX / reasoning leak (MVP):** роль **`fast_text_chat`** и цепочка **`gpt-hub-fast` → `gpt-hub-fallback`** (без `gpt-hub-strong`); `task_type` **`greeting_or_tiny`** в [classifier.py](versions_dep/v3/apps/orchestrator/gpthub_orchestrator/classifier.py); промпты **`v0.5.1`** — запрет вывода «thinking process» в ответ пользователю; тесты `test_classifier_greeting`, обновлённые golden/router. Compose: **`DEFAULT_MODELS`** и **`TASK_MODEL_EXTERNAL`** по умолчанию **`gpt-hub-fast`**. Документация: [OPENWEBUI_ROLES.md](docs/OPENWEBUI_ROLES.md) (Reasoning / Open WebUI), [PROMPT_PRECEDENCE.md](docs/PROMPT_PRECEDENCE.md), [DEMO_PROMPTS.md](docs/DEMO_PROMPTS.md).
