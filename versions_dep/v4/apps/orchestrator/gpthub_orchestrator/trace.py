...
def compute_fallback_used(orchestrator_fallback: dict[str, Any] | None) -> bool:
    """True if orchestrator tried more than one model/key attempt."""
    if not orchestrator_fallback:
        return False
    if orchestrator_fallback.get("mode") == "stream_single_attempt":
        return False
    if orchestrator_fallback.get("mode") == "stream_fallback":
        attempts = orchestrator_fallback.get("attempts") or []
        return len(attempts) > 1
    if int(orchestrator_fallback.get("retries_after_failure") or 0) > 0:
        return True
    attempts = orchestrator_fallback.get("attempts") or orchestrator_fallback.get("model_attempts") or []
    return len(attempts) > 1
...
def build_trace(
    *,
    ...
    openrouter_meta: dict[str, Any] | None = None,
    ...
    trace: dict[str, Any] = {
        ...
        "openrouter_chain": rs.get("openrouter_chain"),
        "model_used": model_used,
        "selected_model": model_used,
        "openrouter_model": model_used,
    ...
    if rs.get("catalog_version") is not None:
        trace["catalog_version"] = rs.get("catalog_version")
    if rs.get("catalog_generated_at") is not None:
        trace["catalog_generated_at"] = rs.get("catalog_generated_at")
    if rs.get("routing_source") is not None:
        trace["routing_source"] = rs.get("routing_source")
    if rs.get("manifest_version") is not None:
        trace["manifest_version"] = rs.get("manifest_version")
    ...
    if openrouter_meta is not None:
        trace["openrouter"] = openrouter_meta
        if openrouter_meta.get("key_index") is not None:
            trace["key_index"] = openrouter_meta.get("key_index")
        if openrouter_meta.get("key_masked") is not None:
            trace["key_masked"] = openrouter_meta.get("key_masked")
        attempts = openrouter_meta.get("model_attempts") or openrouter_meta.get("attempts")
        if attempts is not None:
            trace["model_attempts"] = attempts
        if openrouter_meta.get("quota_remaining") is not None:
            trace["quota_remaining"] = openrouter_meta.get("quota_remaining")
