...
Список бесплатных моделей OpenRouter (курация): `uv run python -m gpthub_orchestrator.tools.list_free_models` (`OPENROUTER_API_KEY` или `--no-auth`). Подсказка цепочки **vision free** под `litellm/config.yaml`: `--suggest-vision-chain [--limit N]`.
