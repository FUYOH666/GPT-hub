...
Trace пишется в **логи** orchestrator и в заголовке **`X-GPTHub-Trace`** (base64 JSON). Ключевые поля: `task_type`, `model_role`, `model_used` / `selected_model`, `prompt_version`, `classifier_source` (сейчас `heuristic`), `attachments_detected`, `fallback_used`, `fallback_aliases`, `orchestrator_fallback` (попытки / `model_selected` / `retries_after_failure`), `server_clock_iso` (если включена подстановка времени). Декодирование: base64 → UTF-8 → JSON. Open WebUI заголовок в UI не показывает — см. [DEMO_PROMPTS.md](../../docs/DEMO_PROMPTS.md) и ROADMAP фаза 4.
