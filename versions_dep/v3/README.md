...
**Роли и маршрутизация:** цепочки LiteLLM-алиасов — [apps/orchestrator/gpthub_orchestrator/data/model_roles.yaml](apps/orchestrator/gpthub_orchestrator/data/model_roles.yaml); **system-промпты по роли** — [apps/orchestrator/gpthub_orchestrator/data/role_prompts.yaml](apps/orchestrator/gpthub_orchestrator/data/role_prompts.yaml) (`prompt_version` в trace). Модели в [v2_c2/litellm/config.yaml](../v2_c2/litellm/config.yaml): текст по умолчанию — локальный **`gpt-hub-turbo`** (и дублирующие алиасы `fast`/`strong`/`doc`/`reasoning-or` на тот же endpoint); **OpenRouter free** — в основном **`gpt-hub-vision`** и **`gpt-hub-fallback`**.
